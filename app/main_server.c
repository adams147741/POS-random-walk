...
static int recv_exact_msg(int fd, uint16_t expected_type, void *payload, uint16_t expected_len) {
    uint16_t type = 0, len = 0;
    if (rw_recv_hdr(fd, &type, &len) < 0) return -1;
    if (type != expected_type || len != expected_len) {
        // consume payload if any to keep stream aligned
        if (len > 0) {
            char tmp[1024];
            uint16_t remaining = len;
            while (remaining > 0) {
                uint16_t chunk = remaining > sizeof(tmp) ? sizeof(tmp) : remaining;
                if (rw_recv_all(fd, tmp, chunk) < 0) break;
                remaining -= chunk;
            }
        }
        return -1;
...
static void run_single_client(int client_fd) {
    // --- HELLO ---
    uint16_t type = 0, len = 0;
    ...
    if (recv_exact_msg(client_fd, RW_MSG_CREATE_SIM, &req, (uint16_t)sizeof(req)) < 0) {
    ...
    printf("server: CREATE ok w=%u h=%u reps=%u K=%u mode=%u world=%u out=%s\n",
    ...
    for (uint32_t i = 0; i <= st.rep_total; i++) {
        ...
        // dummy fill (neskôr nahradíš reálnymi výsledkami)
